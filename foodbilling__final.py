...
Cost_F=Frame(MenuFrame,bg='red',bd=4)
Cost_F.pack(side=BOTTOM)
Drinks_F=Frame(MenuFrame,bg='red',bd=4)
Drinks_F.pack(side=TOP)


Drinks_F=Frame(MenuFrame,bg='red',bd=4,relief=RIDGE)
Drinks_F.pack(side=LEFT)
Food_F=Frame(MenuFrame,bg='red',bd=4,relief=RIDGE)
...
def CostofItem():
    ...
    foc=PriceofFood
    cof.set(foc)
    tp=(PriceofDrinks + PriceofFood + 1.59) * 0.15
    pt.set(tp)
    ut = PriceofDrinks + PriceofFood + 1.59
    st.set(ut)
    cs = 1.59
    sec.set(cs)
    ct = PriceofDrinks + PriceofFood + 1.59 + ((PriceofDrinks + PriceofFood + 1.59) * 0.15)
    tec.set(ct)
    #########################################
...
def update_data():
    ...
    a=Receipt_Ref.get()
    b=str(DateofOrder.get())
    z="20"+b[6:]+"-"+b[3:5]+"-"+b[0:2]
    c=E_Sprite.get()
    d=E_Pepsi.get()
    e=E_DietCoke.get()
    f=E_Mojito.get()
    ...
    if int(c)>0 or int(d)>0 or int(e)>0 or int(f)>0 or int(g)>0 or int(h)>0 or int(i)>0 or int(j)>0 or int(k)>0 or int(l)>0 or int(m)>0 or int(n)>0 or int(o)>0 or \
          int(p)>0 or int(q)>0 or int(r)>0 :
        conn=sq.connect(host="localhost" ,user="root" , passwd="pk123" ,database="fbs")
        mycursor = conn.cursor()
        data=(a,z,int(c),int(d),int(e),int(f),int(g),int(h),int(i),int(j),int(k),int(l),int(m),int(n),int(o),int(p),int(q),int(r),s,t,u,v,w,x)
        mycursor.execute("INSERT INTO MRECORD(Receipt, bdate,Sprite , Pepsi , DietCoke , Mojito , Cappuccino , Fanta , CocaCola , coldcoffee , HogDog , VegBurger, \
                      Pasta , Rice_Plate , Sandwich , Fires , Spagetti , Fazitas , COST_OF_DRINKS , TAXPAID , COST_OF_FOODS , SUBTOTAL, \
                      SERVICE_CHARGE , TOTALCOST ) VALUES('%s', '%s', %d , %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %.2f, %.2f, %.2f, %.2f, %.2f , %.2f)"%(data))
        conn.commit()
    conn.close
...
def show_data():
    ...
    top = Toplevel()
    top.title("Records")
    text = Text(top, width=72, height=30,  padx=10, pady=10,bg='cyan',bd=4,font=('arial',12,'bold'))
    text.pack()
    top.geometry("700x605+0+0")
    conn=sq.connect(host="localhost" ,user="root" , passwd="pk123" ,database="fbs")
    mycursor = conn.cursor()
    mycursor.execute("SELECT * FROM MRECORD")
...
def insert_data():
    ...
    if int(c)>0 or int(d)>0 or int(e)>0 or int(f)>0 or int(g)>0 or int(h)>0 or int(i)>0 or int(j)>0 or int(k)>0 or int(l)>0 or int(m)>0 or int(n)>0 or int(o)>0 or \
        ...
        mycursor.execute("SELECT * FROM MRECORD")
        dftable = mycursor.fetchall()
        dfs=pd.DataFrame(dftable)
...
def prog_fig():
    ...
    top1 = Toplevel()
    top1.title("Progress Report")
    
    top1.geometry("600x405+0+0")
    conn=sq.connect(host="localhost" ,user="root" , passwd="pk123" ,database="fbs")
    mycursor = conn.cursor()

    mycursor.execute("SELECT BDATE, SUM(TOTALCOST) FROM MRECORD GROUP BY BDATE")
    pdata = mycursor.fetchall()
    
    for i in range(len(pdata)):
        lit.append(pdata[i][1])
    ...
    Data2 = {'Date': git,
        'TotalInccomePerDay': lit
       }
    df2 = pd.DataFrame(Data2,columns=['Date', 'TotalInccomePerDay'])
    df2 = df2[['Date', 'TotalInccomePerDay']].groupby('Date').sum()
    print(df2)
    
    figure2 = plt.Figure(figsize=(5,4), dpi=100)
    ax2 = figure2.add_subplot(111)
    line2 = FigureCanvasTkAgg(figure2, top1)
    line2.get_tk_widget().pack(side=TOP, fill=BOTH)
    df2.plot(kind='line', legend=True, ax=ax2, color='r',marker='o', fontsize=10)
    ax2.set_title('Date Vs. Progress Rate')

#########################################Drinks####################################################################
Sprite=Checkbutton(Drinks_F,text='Sprite',variable=var1,onvalue=1,offvalue=0,font=('arial',18,'bold'),
                    bg='red',command=chkSprite).grid(row=0,sticky=W)
Pepsi=Checkbutton(Drinks_F,text='Pepsi',variable=var2,onvalue=1,offvalue=0,font=('arial',18,'bold'),
                    bg='red',command=chkPepsi).grid(row=1,sticky=W)
DietCoke=Checkbutton(Drinks_F,text='DietCoke',variable=var3,onvalue=1,offvalue=0,font=('arial',18,'bold'),
                    bg='red',command=chk_DietCoke).grid(row=2,sticky=W)
Mojito=Checkbutton(Drinks_F,text='Mojito',variable=var4,onvalue=1,offvalue=0,font=('arial',18,'bold'),
                    bg='red',command=chk_Mojito).grid(row=3,sticky=W)
Cappuccino=Checkbutton(Drinks_F,text='Cappuccino',variable=var5,onvalue=1,offvalue=0,font=('arial',18,'bold'),
                    bg='red',command=chk_Cappuccino).grid(row=4,sticky=W)
Fanta=Checkbutton(Drinks_F,text='Fanta',variable=var6,onvalue=1,offvalue=0,font=('arial',18,'bold'),
                    bg='red',command=chk_Fanta).grid(row=5,sticky=W)
CocaCola=Checkbutton(Drinks_F,text='CocaCola',variable=var7,onvalue=1,offvalue=0,font=('arial',18,'bold'),
                    bg='red',command=chk_CocaCola).grid(row=6,sticky=W)
ColdCoffee=Checkbutton(Drinks_F,text='ColdCoffee',variable=var8,onvalue=1,offvalue=0,font=('arial',18,'bold'),
                    bg='red',command=chk_ColdCoffee).grid(row=7,sticky=W)
##############################################Drink Entry###############################################################

txtSprite = Entry(Drinks_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED
                        ,textvariable=E_Sprite)
txtSprite.grid(row=0,column=1)

txtPepsi = Entry(Drinks_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED
                        ,textvariable=E_Pepsi)
txtPepsi.grid(row=1,column=1)

txtDietCoke = Entry(Drinks_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED
                        ,textvariable=E_DietCoke)
txtDietCoke.grid(row=2,column=1)

txtMojito= Entry(Drinks_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED
                        ,textvariable=E_Mojito)
txtMojito.grid(row=3,column=1)

txtCappuccino = Entry(Drinks_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED
                        ,textvariable=E_Cappuccino)
txtCappuccino.grid(row=4,column=1)

txtFanta = Entry(Drinks_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED
                        ,textvariable=E_Fanta)
txtFanta.grid(row=5,column=1)

txtCocaCola = Entry(Drinks_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED
                        ,textvariable=E_CocaCola)
txtCocaCola.grid(row=6,column=1)

txtColdCoffee = Entry(Drinks_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED
...
HotDog = Checkbutton(Food_F,text="HotDog\t\t\t ",variable=var9,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='red',command=chk_HotDog).grid(row=0,sticky=W)
VegBurger = Checkbutton(Food_F,text="VegBurger",variable=var10,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='red',command=chk_VegBurger).grid(row=1,sticky=W)
...
HamBurger = Checkbutton(Food_F,text="Rice Plate ",variable=var12,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='red',command=chk_HamBurger).grid(row=3,sticky=W)
Sandwich = Checkbutton(Food_F,text="Sandwich ",variable=var13,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='red',command=chk_Sandwich).grid(row=4,sticky=W)
...
Spagetti = Checkbutton(Food_F,text="Spagetti ",variable=var15,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='red',command=chk_Spagetti).grid(row=6,sticky=W)
Fazitas = Checkbutton(Food_F,text="Fazitas ",variable=var16,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='red',command=chk_Fazitas).grid(row=7,sticky=W)
################################################Entry Box For Cake##########################################################
txtHotDog=Entry(Food_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED,
                        textvariable=E_HotDog)
txtHotDog.grid(row=0,column=1)

txtVegBurger=Entry(Food_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED,
                        textvariable=E_VegBurger)
txtVegBurger.grid(row=1,column=1)

txtPasta=Entry(Food_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED,
                        textvariable=E_Pasta)
txtPasta.grid(row=2,column=1)

txtHamBurger=Entry(Food_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED,
                        textvariable=E_HamBurger)
txtHamBurger.grid(row=3,column=1)

txtSandwich=Entry(Food_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED,
                        textvariable=E_Sandwich)
txtSandwich.grid(row=4,column=1)

txtFires=Entry(Food_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED,
                        textvariable=E_Fires)
txtFires.grid(row=5,column=1)

txtSpagetti=Entry(Food_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED,
                        textvariable=E_Spagetti)
txtSpagetti.grid(row=6,column=1)

txtFazitas=Entry(Food_F,font=('arial',16,'bold'),bg ='cyan',bd=8,width=6,justify=LEFT,state=DISABLED,
                        textvariable=E_Fazitas)
txtFazitas.grid(row=7,column=1)
###########################################ToTal Cost################################################################################
lblCostofDrinks=Label(Cost_F,font=('arial',14,'bold'),text='Cost of Drinks\t',bg='red',
                fg='black',justify=CENTER)
lblCostofDrinks.grid(row=0,column=0,sticky=W)
txtCostofDrinks=Entry(Cost_F,bg='cyan',bd=7,font=('arial',14,'bold'),
                        insertwidth=2,justify=RIGHT,textvariable=CostofDrinks, state = DISABLED)
txtCostofDrinks.grid(row=0,column=1)

lblCostofFood=Label(Cost_F,font=('arial',14,'bold'),text='Cost of Foods  ',bg='red',
                fg='black',justify=CENTER)
lblCostofFood.grid(row=1,column=0,sticky=W)
txtCostofFood=Entry(Cost_F,bg='cyan',bd=7,font=('arial',14,'bold'),
                        insertwidth=2,justify=RIGHT,textvariable=CostofFood,state = DISABLED)
txtCostofFood.grid(row=1,column=1)

lblServiceCharge=Label(Cost_F,font=('arial',14,'bold'),text='Service Charge',bg='red',
                fg='black',justify=CENTER)
lblServiceCharge.grid(row=2,column=0,sticky=W)
txtServiceCharge=Entry(Cost_F,bg='cyan',bd=7,font=('arial',14,'bold'),
                        insertwidth=2,justify=RIGHT,textvariable=ServiceCharge, state = DISABLED)
txtServiceCharge.grid(row=2,column=1)
###########################################################Payment information###################################################

lblPaidTax=Label(Cost_F,font=('arial',14,'bold'),text='\tPaid Tax',bg='red',bd=7,
                fg='black',justify=CENTER)
lblPaidTax.grid(row=0,column=2,sticky=W)
txtPaidTax=Entry(Cost_F,bg='cyan',bd=7,font=('arial',14,'bold'),
                        insertwidth=2,justify=RIGHT,textvariable=PaidTax, state = DISABLED)
txtPaidTax.grid(row=0,column=3)

lblSubTotal=Label(Cost_F,font=('arial',14,'bold'),text='\tSub Total',bg='red',bd=7,
                fg='black',justify=CENTER)
lblSubTotal.grid(row=1,column=2,sticky=W)
txtSubTotal=Entry(Cost_F,bg='cyan',bd=7,font=('arial',14,'bold'),
                        insertwidth=2,justify=RIGHT,textvariable=SubTotal, state = DISABLED)
txtSubTotal.grid(row=1,column=3)

lblTotalCost=Label(Cost_F,font=('arial',14,'bold'),text='\tTotal',bg='red',bd=7,
                fg='black',justify=CENTER)
lblTotalCost.grid(row=2,column=2,sticky=W)
txtTotalCost=Entry(Cost_F,bg='cyan',bd=7,font=('arial',14,'bold'),
                        insertwidth=2,justify=RIGHT,textvariable=TotalCost, state = DISABLED)
txtTotalCost.grid(row=2,column=3)

#############################################RECEIPT###############################################################################
txtReceipt=Text(Receipt_F,width=46,height=8,bg='cyan',bd=4,font=('arial',12,'bold'))
txtReceipt.grid(row=0,column=0)


###########################################BUTTONS################################################################################
btnTotal=Button(Buttons_F,padx=20,pady=1,bd=7,fg='black',font=('arial',16,'bold'),width=4,text='Total',
                        bg='red',command=CostofItem).grid(row=0,column=0)
btnReceipt=Button(Buttons_F,padx=20,pady=1,bd=7,fg='black',font=('arial',16,'bold'),width=4,text='Receipt',
                        bg='red',command=Receipt).grid(row=0,column=1)
btnReset=Button(Buttons_F,padx=20,pady=1,bd=7,fg='black',font=('arial',16,'bold'),width=4,text='Reset',
                        bg='red',command=Reset).grid(row=0,column=2)
btnExit=Button(Buttons_F,padx=20,pady=1,bd=7,fg='black',font=('arial',16,'bold'),width=4,text='Exit',
                        bg='red',command=iExit).grid(row=0,column=3)
btnUpdata=Button(Buttons_F,padx=20,pady=1,bd=7,fg='black',font=('arial',16,'bold'),width=4,text='Update',
                        bg='red',command=update_data).grid(row=1,column=0)
btnShow=Button(Buttons_F,padx=20,pady=1,bd=7,fg='black',font=('arial',16,'bold'),width=4,text='Showdata',
                        bg='red',command=show_data).grid(row=1,column=1)
btnInsert=Button(Buttons_F,padx=20,pady=1,bd=7,fg='black',font=('arial',16,'bold'),width=4,text='Insertdata',
                        bg='red',command=insert_data).grid(row=1,column=2)
btnProg=Button(Buttons_F,padx=20,pady=1,bd=7,fg='black',font=('arial',16,'bold'),width=4,text='Progress',
                        bg='red',command=prog_fig).grid(row=1,column=3)
